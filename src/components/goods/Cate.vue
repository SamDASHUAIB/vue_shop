  ...
  data() {
    return {
      ...
      addCateForm: {
        ...
        // 分类等级, 默认 1 级
        cat_level: 0,
        // // 分类等级, 默认 1 级 则父分类 id 应该为 0
        cat_pid: 0
      },
      // 添加分类表单的校验规则的对象
      addCateFormRules: {
        cat_name: [{ required: true, message: '请输入分类名称', trigger: 'blur' }, { min: 1, max: 10, message: '长度在 1 到 10 个字符', trigger: 'blur' }]
      },
      // 父级分类的数据列表
      parentCateList: [],
      // 指定级联选择器的配置对象
